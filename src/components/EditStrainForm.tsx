...
interface EditStrainFormProps {
  ...
  onUpdateFavorite: (id: string, strain: Strain, consumptionType: string, dispensary: string) => void;
  onCancel: () => void;
}
...
export default function EditStrainForm({ strains, currentFavorite, onUpdateFavorite, onCancel }: EditStrainFormProps) {
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredStrains, setFilteredStrains] = useState<Strain[]>([]);
  const [selectedStrain, setSelectedStrain] = useState<Strain | null>(null);
  const [consumptionType, setConsumptionType] = useState(currentFavorite.consumptionType || "");
  const [dispensarySearch, setDispensarySearch] = useState(currentFavorite.dispensary || "");
  const [dispensaryResults, setDispensaryResults] = useState<Place[]>([]);
  const [selectedDispensary, setSelectedDispensary] = useState(currentFavorite.dispensary || "");
  ...
  // Initialize selected strain
  useEffect(() => {
    const currentStrain = strains.find(s => s.id === currentFavorite.strainId);
    if (currentStrain) {
      setSelectedStrain(currentStrain);
    }
  }, [currentFavorite.strainId, strains]);
  ...
  const handleSubmit = () => {
    if (selectedStrain) {
      onUpdateFavorite(currentFavorite._id, selectedStrain, consumptionType, selectedDispensary);
    }
  };
  ...
  return (
    ...
    {showStrainInfo && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 max-h-[80vh] overflow-y-auto dark:border-green-700">
